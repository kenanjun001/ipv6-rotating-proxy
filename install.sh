...
func resolveTarget(ctx context.Context,host string,port uint16)(string,string,bool,error){ ps:=strconv.Itoa(int(port)); if ip:=net.ParseIP(host); ip!=nil{if ip.To4()!=nil{return "tcp4",net.JoinHostPort(ip.String(),ps),false,nil}; return "tcp6",net.JoinHostPort(ip.String(),ps),true,nil}; addrs,err:=net.DefaultResolver.LookupIPAddr(ctx,host); if err!=nil{return "","",false,err}; for _,a:=range addrs{if a.IP!=nil&&a.IP.To4()==nil{return "tcp6",net.JoinHostPort(a.IP.String(),ps),true,nil}}; for _,a:=range addrs{if a.IP!=nil&&a.IP.To4()!=nil{return "tcp4",net.JoinHostPort(a.IP.String(),ps),false,nil}}; return "","",false,fmt.Errorf("目标 %s 没有可用 IP 记录",host)}
...
func proxy(client net.Conn,host string,port uint16,isSocks bool){ start:=time.Now(); cip:=client.RemoteAddr().String(); target:=net.JoinHostPort(host,strconv.Itoa(int(port))); ip:=getIP(); if ip==nil{addLog(cip,target,"无","失败: 无可用 IP",time.Since(start)); if isSocks{client.Write([]byte{5,1,0,1,0,0,0,0,0,0})}else{client.Write([]byte("HTTP/1.1 503 Service Unavailable\r\n\r\n"))}; atomic.AddInt64(&stats.FailedConns,1); return}; id:=fmt.Sprintf("%s-%d",cip,time.Now().UnixNano()); activeLock.Lock(); active[id]=&ActiveConn{ID:id,ClientIP:cip,Target:target,IPv6:ip.String(),StartTime:start}; activeLock.Unlock(); defer func(){activeLock.Lock(); delete(active,id); activeLock.Unlock()}(); ctx,cancel:=context.WithTimeout(context.Background(),30*time.Second); defer cancel(); network,dialTarget,useIPv6,err:=resolveTarget(ctx,host,port); if err!=nil{st:="失败: "+err.Error(); if len(st)>80{st=st[:80]}; addLog(cip,target,ip.String(),st,time.Since(start)); if isSocks{client.Write([]byte{5,4,0,1,0,0,0,0,0,0})}else{client.Write([]byte("HTTP/1.1 502 Bad Gateway\r\n\r\n"))}; atomic.AddInt64(&stats.FailedConns,1); return}; d:=net.Dialer{Timeout:15*time.Second}; logIP:=ip.String(); if useIPv6{d.LocalAddr=&net.TCPAddr{IP:ip}}else{logIP="IPv4出口"}; remote,err:=d.DialContext(ctx,network,dialTarget); if err!=nil{st:="失败: "+err.Error(); if len(st)>80{st=st[:80]}; addLog(cip,target,logIP,st,time.Since(start)); if isSocks{client.Write([]byte{5,4,0,1,0,0,0,0,0,0})}else{client.Write([]byte("HTTP/1.1 502 Bad Gateway\r\n\r\n"))}; atomic.AddInt64(&stats.FailedConns,1); if useIPv6&&config.AutoClean{select{case discardQueue<-ip:default:{}}}; return}; defer remote.Close(); atomic.AddInt64(&stats.SuccessConns,1); addLog(cip,target,logIP,"成功",time.Since(start)); if isSocks{client.Write([]byte{5,0,0,1,0,0,0,0,0,0})}else{client.Write([]byte("HTTP/1.1 200 Connection Established\r\n\r\n"))}; deadline:=start.Add(1000*time.Second); var wg sync.WaitGroup; wg.Add(2); go copyConn(remote,client,deadline,&stats.TotalBytesSent,&wg); go copyConn(client,remote,deadline,&stats.TotalBytesRecv,&wg); wg.Wait() }
