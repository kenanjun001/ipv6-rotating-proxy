...
INSTALL_DIR="/home/ipv6-proxy"
BUILD_DIR="/home/ipv6-proxy-build"
GO_VERSION="1.21.5"
...
func randText(prefix string,n int) string{ const letters="abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789"; if n<1{n=12}; b:=make([]byte,n); rb:=make([]byte,n); if _,e:=rand.Read(rb); e!=nil{ for i:=range b{rngLock.Lock(); b[i]=letters[rng.Intn(len(letters))]; rngLock.Unlock()} }else{ for i,x:=range rb{ b[i]=letters[int(x)%len(letters)] } }; return prefix+string(b) }
func publicHost(r *http.Request) string{ h:=r.Host; if h==""{return "服务器IP"}; if strings.Contains(h,":"){if host,_,e:=net.SplitHostPort(h); e==nil{return host}}; return h }
...
func firstSetup() error{ config.WebUsername=readLine("Web 账号","admin"); config.WebPassword=readLine("Web 密码","admin123"); config.Port=readLine("代理端口","1080"); config.WebPort=readLine("Web 端口","8080"); config.Username=readLine("代理用户名",randText("u_",10)); config.Password=readLine("代理密码",randText("p_",18)); l,err:=selectInterface(); if err!=nil{return err}; config.Interface=l.Attrs().Name; p,err:=selectPrefix(l); if err!=nil{return err}; config.IPv6Prefix=p; config.NextHopAddr=readLine("下一跳地址，没有则留空",""); config.InitialPool=readInt("初始 IP 数量",1); config.TargetPool=readInt("目标 IP 数量",1); if config.TargetPool<config.InitialPool{config.TargetPool=config.InitialPool}; config.AutoRotate=strings.ToLower(readLine("启用自动轮换? (y/n)","n"))=="y"; if config.AutoRotate{config.AutoRotateHours=readInt("轮换间隔小时",6)}; config.AutoClean=strings.ToLower(readLine("启用失败 IP 自动清理? (y/n)","y"))=="y"; return saveConfig() }
GOEOF
cat <<'GOEOF' >> main.go
...
func startWeb(ctx context.Context)*http.Server{mux:=http.NewServeMux(); mux.HandleFunc("/",webAuth(index)); mux.HandleFunc("/api/stats",webAuth(func(w http.ResponseWriter,r *http.Request){up:=time.Since(stats.StartTime); host:=publicHost(r); js(w,map[string]interface{}{"active":atomic.LoadInt64(&stats.ActiveConns),"total":atomic.LoadInt64(&stats.TotalConns),"success":atomic.LoadInt64(&stats.SuccessConns),"failed":atomic.LoadInt64(&stats.FailedConns),"pool":atomic.LoadInt64(&stats.PoolSize),"target":config.TargetPool,"bytes_recv":atomic.LoadInt64(&stats.TotalBytesRecv),"bytes_sent":atomic.LoadInt64(&stats.TotalBytesSent),"auto_rotate":config.AutoRotate,"rotate_interval":config.AutoRotateHours,"uptime":fmt.Sprintf("%dd %dh %dm",int(up.Hours())/24,int(up.Hours())%24,int(up.Minutes())%60),"proxy_port":config.Port,"proxy_username":config.Username,"proxy_password":config.Password,"proxy_host":host,"http_proxy":"http://"+config.Username+":"+config.Password+"@"+host+":"+config.Port,"socks5_proxy":"socks5://"+config.Username+":"+config.Password+"@"+host+":"+config.Port})})); mux.HandleFunc("/api/ping",webAuth(func(w http.ResponseWriter,r *http.Request){js(w,map[string]interface{}{"ok":true,"time":time.Now().Format(time.RFC3339),"port":config.Port})})); mux.HandleFunc("/api/rotate_auth",webAuth(func(w http.ResponseWriter,r *http.Request){if r.Method!="POST"{http.Error(w,"只支持 POST",405);return}; config.Username=randText("u_",10); config.Password=randText("p_",18); if e:=saveConfig(); e!=nil{http.Error(w,e.Error(),500);return}; host:=publicHost(r); js(w,map[string]interface{}{"message":"代理账号密码已更新","username":config.Username,"password":config.Password,"http_proxy":"http://"+config.Username+":"+config.Password+"@"+host+":"+config.Port,"socks5_proxy":"socks5://"+config.Username+":"+config.Password+"@"+host+":"+config.Port})})); mux.HandleFunc("/api/rotate",webAuth(func(w http.ResponseWriter,r *http.Request){if r.Method!="POST"{http.Error(w,"只支持 POST",405);return}; go rotatePool(); js(w,map[string]string{"message":"轮换已开始"})})); mux.HandleFunc("/api/add_ips",webAuth(func(w http.ResponseWriter,r *http.Request){if r.Method!="POST"{http.Error(w,"只支持 POST",405);return}; var q struct{Count int `json:"count"`}; _=json.NewDecoder(r.Body).Decode(&q); if q.Count<1{q.Count=1}; added:=addPoolIPs(q.Count); js(w,map[string]interface{}{"message":"新增完成","added":added,"pool":atomic.LoadInt64(&stats.PoolSize),"target":config.TargetPool})})); mux.HandleFunc("/api/pool",webAuth(func(w http.ResponseWriter,r *http.Request){if r.Method!="POST"{http.Error(w,"只支持 POST",405);return}; var q struct{Count int `json:"count"`}; _=json.NewDecoder(r.Body).Decode(&q); if q.Count<1{http.Error(w,"count 必须大于 0",400);return}; pool:=setPoolTarget(q.Count); js(w,map[string]interface{}{"message":"数量已更新","pool":pool,"target":config.TargetPool})})); mux.HandleFunc("/api/clear_ips",webAuth(func(w http.ResponseWriter,r *http.Request){if r.Method!="POST"{http.Error(w,"只支持 POST",405);return}; deleted:=clearAllIPv6(); js(w,map[string]interface{}{"message":"IPv6 已全部删除","deleted":deleted,"target":config.TargetPool})})); mux.HandleFunc("/api/active",webAuth(func(w http.ResponseWriter,r *http.Request){activeLock.RLock(); arr:=[]*ActiveConn{}; now:=time.Now(); for _,x:=range active{x.Duration=fmt.Sprintf("%.1fs",now.Sub(x.StartTime).Seconds()); arr=append(arr,x)}; activeLock.RUnlock(); js(w,arr)})); mux.HandleFunc("/api/logs",webAuth(func(w http.ResponseWriter,r *http.Request){logsLock.RLock(); arr:=append([]*ConnLog(nil),logs...); logsLock.RUnlock(); js(w,arr)})); srv:=&http.Server{Addr:":"+config.WebPort,Handler:mux}; go func(){<-ctx.Done(); srv.Shutdown(context.Background())}(); go func(){log.Printf("Web 面板: http://0.0.0.0:%s",config.WebPort); if e:=srv.ListenAndServe(); e!=nil&&e!=http.ErrServerClosed{log.Printf("Web 服务错误: %v",e)}}(); return srv }
...
<style>
:root{--bg:#f5f1e8;--ink:#1f2933;--muted:#6b665c;--line:#ded6c8;--card:#fffaf0;--soft:#ede4d3;--green:#2f7d57;--blue:#34699a;--red:#b94a48;--amber:#b7791f}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top left,#fffaf0 0,#f5f1e8 35%,#eee5d6 100%);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",Arial,sans-serif}.wrap{max-width:1160px;margin:0 auto;padding:28px 18px 42px}.top{display:flex;justify-content:space-between;gap:16px;align-items:flex-end;margin-bottom:18px}.eyebrow{color:var(--muted);font-size:13px;letter-spacing:.08em}.title{font-size:30px;font-weight:800;margin:4px 0}.subtitle{color:var(--muted);font-size:14px}.pill{border:1px solid var(--line);background:rgba(255,250,240,.8);border-radius:999px;padding:9px 13px;color:var(--muted);white-space:nowrap}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.card,.box{background:rgba(255,250,240,.92);border:1px solid var(--line);border-radius:18px;box-shadow:0 14px 35px rgba(70,54,30,.08)}.card{padding:16px}.label{font-size:13px;color:var(--muted)}.num{font-size:28px;font-weight:800;margin-top:8px}.box{padding:18px;margin-top:14px}h2{font-size:18px;margin:0 0 14px}.controls{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.field{display:flex;gap:8px;align-items:center;background:var(--soft);border-radius:14px;padding:10px}.field label{font-size:13px;color:var(--muted);min-width:70px}input{width:100%;border:1px solid var(--line);background:#fffdf7;color:var(--ink);border-radius:10px;padding:10px;font-size:14px}.btn,button{border:0;border-radius:11px;padding:10px 13px;font-weight:700;color:white;cursor:pointer;white-space:nowrap;transition:.15s transform,.15s opacity}.btn:hover,button:hover{transform:translateY(-1px);opacity:.92}.primary{background:var(--blue)}.ok{background:var(--green)}.warn{background:var(--amber)}.danger{background:var(--red)}.ghost{background:#75685a}.status{min-height:22px;color:var(--muted);font-size:13px;margin-top:10px}.proxy-row{display:grid;grid-template-columns:110px 1fr auto auto;gap:8px;align-items:center;margin:9px 0}.code{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;background:#27231d;color:#fff6dc;border-radius:12px;padding:11px;overflow:auto;font-size:13px}.small{font-size:12px;color:var(--muted)}table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid var(--line);padding:10px 8px;text-align:left;font-size:13px;word-break:break-all}th{color:var(--muted);font-weight:700}.split{display:grid;grid-template-columns:1.1fr .9fr;gap:14px}@media(max-width:820px){.top,.split{display:block}.grid{grid-template-columns:repeat(2,1fr)}.proxy-row{grid-template-columns:1fr}.pill{display:inline-block;margin-top:12px}}@media(max-width:520px){.grid{grid-template-columns:1fr}.title{font-size:24px}.wrap{padding:18px 12px}}
</style>
</head>
<body>
<div class="wrap">
  <div class="top"><div><div class="eyebrow">OCI IPv6 Proxy</div><div class="title">代理池控制台</div><div class="subtitle">少一点花哨，多一点好用。复制、连通性测试和账号轮换都放在手边。</div></div><div class="pill">面板延迟 <b id="panelLatency">-</b></div></div>
  <div class="grid"><div class="card"><div class="label">活跃连接</div><div id="active" class="num">-</div></div><div class="card"><div class="label">总连接</div><div id="total" class="num">-</div></div><div class="card"><div class="label">成功</div><div id="success" class="num">-</div></div><div class="card"><div class="label">失败</div><div id="failed" class="num">-</div></div><div class="card"><div class="label">IP 池</div><div id="pool" class="num">-</div></div><div class="card"><div class="label">目标数量</div><div id="target" class="num">-</div></div><div class="card"><div class="label">总流量</div><div id="traffic" class="num" style="font-size:21px">-</div></div><div class="card"><div class="label">运行时间</div><div id="uptime" class="num" style="font-size:18px">-</div></div></div>
  <div class="split"><div class="box"><h2>代理信息</h2><div class="proxy-row"><b>HTTP</b><div id="httpProxy" class="code">-</div><button class="ghost" onclick="copyText('httpProxy',this)">复制</button><button class="primary" onclick="testLocal('http')">本地测试</button></div><div class="proxy-row"><b>SOCKS5</b><div id="socksProxy" class="code">-</div><button class="ghost" onclick="copyText('socksProxy',this)">复制</button><button class="primary" onclick="testLocal('socks')">本地测试</button></div><div class="small">测试按钮由你的浏览器发起请求，测的是你当前网络到这台服务器代理端口的连通和延迟，不走服务器本机网络。</div><div id="testStatus" class="status"></div><button class="warn" onclick="rotateAuth()">随机更换代理账号密码</button></div>
  <div class="box"><h2>IP 池控制</h2><div class="controls"><div class="field"><label for="addCount">增加</label><input id="addCount" type="number" min="1" step="1" value="1"><button class="ok" onclick="addIps()">增加</button></div><div class="field"><label for="poolCount">数量</label><input id="poolCount" type="number" min="1" step="1" value="1"><button class="primary" onclick="setPool()">修改</button></div><div class="field"><label>轮换</label><button class="warn" onclick="rotateNow()">立即轮换</button></div><div class="field"><label>清空</label><button class="danger" onclick="clearIps()">删除全部</button></div></div><div id="opStatus" class="status"></div></div></div>
  <div class="box"><h2>实时连接 <span class="small">最多 10 条</span></h2><table><thead><tr><th>客户端</th><th>目标</th><th>出口</th><th>时长</th></tr></thead><tbody id="act"></tbody></table></div>
  <div class="box"><h2>最近日志</h2><table><thead><tr><th>时间</th><th>客户端</th><th>目标</th><th>出口</th><th>状态</th><th>耗时</th></tr></thead><tbody id="logs"></tbody></table></div>
</div>
<script>
let lastStats=null;async function j(u,o){let r=await fetch(u,o);if(!r.ok)throw new Error(await r.text());return r.json()}function fmt(n){n=Number(n||0);let u=['B','KB','MB','GB','TB'],i=0;while(n>=1024&&i<u.length-1){n/=1024;i++}return (i?n.toFixed(2):n.toFixed(0))+' '+u[i]}function rows(id,a,c){a=(a||[]).slice(-10);document.getElementById(id).innerHTML=a.length?a.map(x=>'<tr>'+c.map(f=>'<td>'+(f(x)||'')+'</td>').join('')+'</tr>').join(''):'<tr><td colspan="6" class="small">暂无</td></tr>'}function post(u,d){return j(u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d||{})})}function val(id){return Math.max(1,parseInt(document.getElementById(id).value||'1',10))}function status(id,t){document.getElementById(id).textContent=t}async function copyText(id,btn){let text=document.getElementById(id).textContent.trim();try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(text)}else{let ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.left='-9999px';document.body.appendChild(ta);ta.focus();ta.select();if(!document.execCommand('copy'))throw new Error('copy failed');ta.remove()}let old=btn.textContent;btn.textContent='已复制';setTimeout(()=>btn.textContent=old,1200)}catch(e){status('testStatus','复制失败：浏览器禁止剪切板权限，请手动选中复制')}}async function pingPanel(){let t=performance.now();try{await j('/api/ping?ts='+Date.now(),{cache:'no-store'});document.getElementById('panelLatency').textContent=Math.round(performance.now()-t)+' ms'}catch(e){document.getElementById('panelLatency').textContent='失败'}}async function testLocal(kind){if(!lastStats)return;let url=location.protocol+'//'+(lastStats.proxy_host||location.hostname)+':'+lastStats.proxy_port+'/?probe='+Date.now();let t=performance.now();status('testStatus','正在从你的浏览器测试 '+kind.toUpperCase()+' 端口...');try{await fetch(url,{mode:'no-cors',cache:'no-store'});status('testStatus',kind.toUpperCase()+' 端口可达，延迟约 '+Math.round(performance.now()-t)+' ms')}catch(e){status('testStatus',kind.toUpperCase()+' 端口不可达或被本地网络/浏览器拦截：'+e.message)}}async function rotateAuth(){if(!confirm('确定随机更换代理用户名和密码？旧链接会立刻失效。'))return;try{let r=await post('/api/rotate_auth',{});status('opStatus',r.message);await tick()}catch(e){status('opStatus','更换失败：'+e.message)}}async function addIps(){try{let r=await post('/api/add_ips',{count:val('addCount')});status('opStatus','已新增 '+r.added+' 个 IPv6，目标数量 '+r.target);tick()}catch(e){status('opStatus','操作失败: '+e.message)}}async function setPool(){try{let r=await post('/api/pool',{count:val('poolCount')});status('opStatus','IP 数量已修改为 '+r.target+'，当前池 '+r.pool);tick()}catch(e){status('opStatus','操作失败: '+e.message)}}async function rotateNow(){try{let r=await post('/api/rotate',{});status('opStatus',r.message||'轮换已开始');setTimeout(tick,1500)}catch(e){status('opStatus','操作失败: '+e.message)}}async function clearIps(){if(!confirm('确定删除全部 IPv6 地址吗？'))return;try{let r=await post('/api/clear_ips',{});status('opStatus','已删除 '+r.deleted+' 个 IPv6，目标数量已设为 '+r.target);tick()}catch(e){status('opStatus','操作失败: '+e.message)}}async function tick(){try{let s=await j('/api/stats');lastStats=s;for(let k of ['active','total','success','failed','pool','target','uptime'])document.getElementById(k).textContent=s[k];document.getElementById('httpProxy').textContent=s.http_proxy||'-';document.getElementById('socksProxy').textContent=s.socks5_proxy||'-';if(document.activeElement.id!='poolCount')document.getElementById('poolCount').value=(s.target??s.pool??1);document.getElementById('traffic').textContent=fmt((s.bytes_sent||0)+(s.bytes_recv||0));rows('act',await j('/api/active'),[x=>x.ClientIP||x.client_ip,x=>x.Target||x.target,x=>x.IPv6||x.ipv6,x=>x.Duration||x.duration]);rows('logs',await j('/api/logs'),[x=>x.Time||x.time,x=>x.ClientIP||x.client_ip,x=>x.Target||x.target,x=>x.IPv6||x.ipv6,x=>x.Status||x.status,x=>x.Duration||x.duration])}catch(e){}}
setInterval(tick,3000);setInterval(pingPanel,5000);tick();pingPanel();
</script>
</body>
</html>
HTMLEOF
